  ...
  for (let button of buttons) {
    button.addEventListener("click", function () {
      ...
      localStorage.setItem(
        "chord_sequencer_mode",
        JSON.stringify(chordSequencerMode)
      );
...
function noteLength() {
  ...
  noteLengthSelected.addEventListener("click", () => {
    let noteLengthIndex = noteLengthArray.indexOf(
      noteLengthSelected.textContent
    );
    if (noteLengthIndex < 3) {
      noteLengthIndex++;
    } else {
      noteLengthIndex = 0;
    }
    ...
    localStorage.setItem(
      "chord_sequencer_note",
      JSON.stringify(chord_sequencer_note)
    );
  });
}
...
function displayModeChords(filteredModeChords, modeNameToFilter) {
  for (let i = 0; i < filteredModeChords.length; i++) {
    document.getElementById(`deg${i + 1}`).innerHTML =
      filteredModeChords[i].toUpperCase();
    document.getElementById(
      `deg${i + 1}`
    ).classList = `button ${modeNameToFilter[i]}`;
  }
}
...
and triggers the sound */
  function triggerNote(note) {
    ...
    let selectedNoteLengthtoPlay =
      noteLengthObject[
        document.getElementById("select-note-length").textContent
      ];
